...
for fn in filenames:
    ...
    columns_to_add = ['欄位名稱(中)', '欄位名稱(英)', '中文值標籤', '序號', '型態', '簡易說明', '備註', '資料變動說明', '處理後刪除', '新增欄位', '資料表名稱', '檢誤邏輯', '資料庫簡介', '資料涵蓋學年度', '欄位勾選', '年份選取', '層級']
    sheet_fn = sheet_fn.reindex(columns = [*sheet_fn.columns, *columns_to_add], fill_value = np.nan)
    # Fill in 層級
    unit = '以「校'
    if unit in fn: 
        sheet_fn['層級'] = '校'
    ...
    for s in sn:
        ...
        sheet_fn.loc[(sheet_fn['原始欄位名稱'] == s) & (len(vallabs) < 10) & (csv_fn[s].dtype == object), '中文值標籤'] = '; '.join(vallabs)
    # Fill in 資料表名稱
    sheet_fn['資料表名稱'] = fn
    # Reorder
    sheet_fn = sheet_fn.loc[:, ['欄位名稱(中)', '原始欄位名稱', '欄位名稱(英)', '中文值標籤', '序號', '型態', '簡易說明', '備註', '資料變動說明', '處理後刪除', '新增欄位', '資料表名稱', '檢誤邏輯', '資料庫簡介', '資料涵蓋學年度', '欄位勾選', '年份選取', '層級']]
    # Export
    book = load_workbook(f'{local_directory}Research.xlsx')
    writer = pd.ExcelWriter(f'{local_directory}Research.xlsx', engine='openpyxl') 
    writer.book = book
...
for fn in filenames:
    ...
    writer = pd.ExcelWriter(f'{local_directory}Student.xlsx', engine='openpyxl') 
    writer.book = book
    sheet_fn.to_excel(writer, sheet_name= fn[:5])
    writer.save()
    writer.close()
